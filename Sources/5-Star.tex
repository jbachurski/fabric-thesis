...
In this chapter, I describe the following contributions: \begin{itemize}
    ...
    \item A formalisation of with an operational semantics (Section \ref{subsec:semantics}), type system (Section \ref{sec:typing};  \emph{without parametric polymorphism}), and type safety (Section \ref{subsec:type-safety}).
    \item Finally, in Section \ref{subsec:type-inference} I describe how to bestow Star (extended with parametric polymorphism) with ML-style type inference in terms of the framework from Chapter \ref{algebraic-subtyping}.
\end{itemize}
...
The array shape values (and types in Section~\ref{sec:typing}) in Star are a novel contribution.
...
\begin{figure}
    ...
    \caption{
        ...
        We take the union of sets of dimensions $\ell$ (defaulting $s_\ell' = s_\ell''$ is either is absent), but equate components $T$. 
...
I focus on the approach to typing array shapes and indices, and finish with a statement of type safety for Star in Section \ref{subsec:type-safety} and application of the type inference framework (Chapter \ref{algebraic-subtyping}) to Star in Section~\ref{subsec:type-inference}.
...
\begin{figure}[p]
    ...
    Quantifiers $\forall t \ldotp \cdots$ stand for an expanded list of assumptions instantiated for all appropriate $t$.
    }
    \label{fig:typing}
\end{figure}

\paragraph{Shape and index types} Recall that in Section \ref{subsec:shape-bounds} we determined that integers index into sized shapes, records into product shapes, and variants into concatenation shapes. This happens both at the value- and type-level, with examples in Table \ref{tab:shape-index-examples}.

While in basic array type systems it is sufficient to presume that the type of a shape and a matching index are the same, in Star this is clearly not the case. For this purpose, we introduce a $\iota$ metafunction on types (defined in Figure \ref{fig:iota}): given a shape type $\sigma$, $\eta = \iota(\sigma)$ is the supertype of all matching index types for $\sigma$.

Constructing $\iota$ is not so surprising -- similar concepts often arise when generalising array-like structures, e.g.\@ \textit{containers} \cite{containers} or Naperian (representable) functors, which \textcite{naperian-apl} has found useful for statically modelling certain patterns of array programming. To the author's knowledge, neither of these have yet lead to the key ideas present in Star.
...
Indeed, $\vess' \broadcastsem \vess''$ (for shape values $\vess': \sigma'$, $\vess'' : \sigma''$) is a partial operator that finds the shape $\vess : \sigma$ for which $\sigma = \sigma' \meet \sigma''$ (where $\meet$ is the meet on types agreeing with $\sub$), so that for any index $v$: 
$$ v \inbs \vess \iff v \inbs \vess' \text{ and } v \inbs \vess'' $$
Hence, $\broadcast$ is the meet in a lattice spanned by the predicate $\inbs s$, and its result type takes the meet of shape types: clarifying how broadcasting finds `sub-shapes', as said in Section~\ref{subsec:array-shapes}.
...
I prove these theorems in Appendix \ref{star-type-safety-proof}, but they follow straightforwardly by structural induction -- the key lemma is that we can show a notion of agreement between the subtyping relation $\sub$ and the structurally-in-bounds relation $\inbstr$.


\subsection{Type inference \& polymorphism}
\label{subsec:type-inference}

It turns out that Star's subtyping relation both forms a distributive lattice, and $\iota$ forms a type lattice homomorphism.
...
It is worth noting that shape polymorphism (when we exclude sized shapes) gives us index safety for free, akin to \textcite{theorems-for-free}.
