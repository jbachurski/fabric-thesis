...
In this chapter (based on a paper of the same name \cite{star}), I describe the following contributions: \begin{itemize}
    ...
    \item Finally, in Section~\ref{subsec:type-inference} I describe how to bestow \starr{} (\emph{with parametric polymorphism}) with ML-style type inference using \inference{} (Chapter~\ref{algebraic-subtyping}), testing it in practice with \compiler{} (Chapter~\ref{fabric}).
\end{itemize}
...
I first focus on the typing of array shapes and indices.\todo[color=red]{FL-style records?}
...
It turns out that \starr{}'s subtyping relation both forms a distributive lattice\todo[color=green]{closure!}, and $\iota$ forms a type algebra isomorphism.
...
To display the features of \starr{} and its elegance in expressing common array programming patterns, we consider a case study of an array program -- and contrast it to presently used languages.\todo[color=red]{todo}
