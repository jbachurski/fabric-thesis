...
\newcommand{\flftop}{\boxed{\top}}
\newcommand{\flfbot}{\boxed{\bot}}
\newnotation{flftop}{$\flftop$}{Top field type}{L048}
\newnotation{flfbot}{$\flfbot$}{Bottom field type}{L049}
...
\newnotation{meet}{$\meet$}{Meet in the lattice of types}{S030}

\newcommand{\join}[0]{\lor}
\newnotation{join}{$\super$}{Join in the lattice of types}{S035}

\newcommand{\comp}[0]{\lnot}
\newnotation{comp}{$\comp$}{Negation in the Boolean lattice of types}{S040}

\newcommand{\subsume}[0]{\sub^\forall}
\newnotation{subsume}{$\subsume$}{Subsumption of type schemes}{S050}

\newcommand{\morph}{\zeta}
\newnotation{morphism}{$\morph$}{Homomorphism in the type lattice}{S060}

\newcommand{\ladj}[1]{\overleftarrow{#1}}
\newnotation{ladj}{$\protect\ladj{\morph}$}{Left adjoint of a lattice homomorphism}{S070}

\newcommand{\radj}[1]{\overrightarrow{#1}}
\newnotation{radj}{$\protect\radj{\morph}$}{Right adjoint of a lattice homomorphism}{S075}

\newcommand{\ladjrest}[1]{\underline{\overleftarrow{#1}}}
\newnotation{ladjrest}{$\protect\ladjrest{\morph}$}{Remainder of a left adjoint of a lattice homomorphism}{S080}

\newcommand{\radjrest}[1]{\underline{\overrightarrow{#1}}}
\newnotation{radjrest}{$\protect\radjrest{\morph}$}{Remainder of a right adjoint of a lattice homomorphism}{S085}
...
\newnotation{constr}{$c$}{Constraint}{T3}
\newnotation{constrctx}{$E$}{Constraint rewriting context}{T6}

\newcommand{\und}[0]{\mathop{\&}}
\newnotation{und}{$\comp$}{Conjunction of constraints}{T10}
...
\newnotation{iota}{$\iota$}{Index of shape: upper bound on index type given shape type}{Z023}
...
\newcommand{\sarrayval}[2]{\exprcolor{\textsc{Arr}(#1,#2)}}
...
\newcommand{\sproductval}[1]{\shapeexprcolor{\left\{\!\left|{#1}\right|\!\right\}}}
...
\newcommand{\sproducttype}[1]{\shapetypecolor{\left\{\!\left|{#1}\right|\!\right\}}}
\newcommand{\sconcattype}[1]{\shapetypecolor{\left\llbracket{#1}\right\rrbracket}}

\newcommand{\evalct}[2]{{#1}\left\langle{#2}\right\rangle}
\newcommand{\evalctx}{C\langle\Box\rangle}
\newnotation{evalctx}{$\evalctx$}{Evaluation context -- with hole $\Box$ for substitution}{Z070}
